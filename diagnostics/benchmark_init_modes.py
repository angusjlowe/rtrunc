"""
Plots L1 error vs iteration for getWeightsFromCoverage with different
initialisations, on k-DPP marginals derived from power-law weights.

Fixed n=N, k=n//2. Gammas [0.1, 1.0, 10.0] go from nearly-uniform to
highly concentrated, recreating the large-first-entry oscillation for
logit init at large gamma.
"""
...
def run_benchmark():
    ...
    for gamma in GAMMAS:
        ...
        results[gamma] = (hist0, hist1, histv2, histv3)
    return results
...
def plot_results(results):
    ...
    for i, gamma in enumerate(GAMMAS):
        ax = axes[i]
        hist0, hist1, histv2, histv3 = results[gamma]
    ...
    fig.tight_layout()
    plot_path = os.path.join(os.path.dirname(__file__), "benchmark_init_modes.pdf")
